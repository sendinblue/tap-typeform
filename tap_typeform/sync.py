...
def sync(client, config, state, catalog):
    ...
    for stream in streams_to_sync:
        ...
        if stream == 'forms' and stream in selected_streams:
            ...
            stream_obj.sync_obj(client, state, catalog['streams'], config["start_date"], selected_streams, records_count, config["schemaless"])
        elif not stream_obj.parent:
            write_schemas(stream, catalog, selected_streams)

            for form in _forms_to_list(config):
                stream_obj.sync_obj(client, state, catalog['streams'], form, config["start_date"], selected_streams, records_count, config["schemaless"])
